...
func process(w http.ResponseWriter, r *http.Request) {
	...
	argsQuant := []string{
		runnerQuant, "--input", dstPath,
		"--version", qasmVersion, "--layers", itoa(qaoaLayers),
		"--iterations", itoa(iterations), "--baseline-limit", itoa(baselineLimit),
		"--demo-batch", itoa(demoBatch),
	}
	...
	_ = json.Unmarshal(outClass, &rClass)
	_ = json.Unmarshal(outQuant, &rQuant)
	...
	for i := 0; i < min(len(rClass.Results), len(rQuant.Results)); i++ {
		...
		perGraph = append(perGraph, map[string]any{
