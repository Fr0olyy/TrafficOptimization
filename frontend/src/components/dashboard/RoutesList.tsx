...
import { ChevronDown, ChevronUp, MapPin, Zap, Clock, Cpu, Route, Gauge, Brain } from 'lucide-react';
        ...
        return (
            <div className="routes-empty-state">
                ...
                <p className="empty-description">
                    Route data will appear here after processing your traffic optimization problem.
                </p>
            </div>
        );
    }

    const getQuantumEfficiency = (quantumCost: number, classicalCost?: number | null) => {
    ...
    return (
        <div className="routes-list">
            {routes.map((route) => {
                ...
                const efficiency = getQuantumEfficiency(route.quantum.cost, route.mirea?.cost);
                const hasAdvantage = efficiency > 0;

                return (
                    <div
                        key={route.route_index}
                        className={`route-card ${isExpanded ? 'expanded' : ''} ${hasAdvantage ? 'advantage' : ''}`}
                    >
                        {/* Header */}
                        <div
                        ...
                        >
                            ...
                            <div className="route-status">
                                {route.mirea && (
                                    <div className={`efficiency-badge ${hasAdvantage ? 'positive' : 'neutral'}`}>
                                        <Gauge className="efficiency-icon" />
                                        <span>{hasAdvantage ? '+' : ''}{efficiency.toFixed(1)}%</span>
                                    </div>
                                )}
                                        ...
                                        <div className="quantum-params">
                                            <div className="param-card">
                                                ...
                                                <div className="param-content">
                                                    <div className="param-value">{route.quantum.p_layers}</div>
                                            ...
                                            <div className="param-card">
                                                ...
                                                <div className="param-content">
                                                    <div className="param-value">{route.quantum.qubits}</div>
                                            ...
                                            <div className="param-card">
                                                ...
                                                <div className="param-content">
                                                    <div className="param-value">{(route.quantum.time * 1000).toFixed(0)}</div>
                                        ...
                                        <div className="solution-section classical">
                                            ...
                                            <div className="path-visualization">
                                                ...
                                                <div className="path-nodes">
                                                    {route.mirea.path.map((node, idx) => (
                                                        <div key={idx} className="path-step">
                                                            <div className="node-circle classical">{node}</div>
                                                            {idx < route.mirea.path.length - 1 && (
                                                                <div className="connection-line" />
                                                            )}
                                                        </div>
                                                    ))}
                                            ...
                                            <div className="comparison-stats">
                                                <div className="comparison-item">
                                                    <span className="comparison-label">Cost Difference:</span>
                                                    <span className={`comparison-value ${hasAdvantage ? 'positive' : 'neutral'}`}>
                                                        {hasAdvantage ? '-' : '+'}{Math.abs(route.mirea.cost - route.quantum.cost).toFixed(2)}
                                                    </span>
                                                </div>
                                                <div className="comparison-item">
                                                    <span className="comparison-label">Time Difference:</span>
                                                    <span className="comparison-value positive">
                                                        -{Math.abs(route.mirea.time - route.quantum.time).toFixed(3)}s
                                                    </span>
                                                </div>
                                                <div className="comparison-item">
                                                    <span className="comparison-label">Quantum Efficiency:</span>
                                                    <span className={`comparison-value ${hasAdvantage ? 'positive' : 'neutral'}`}>
                                                        {hasAdvantage ? '+' : ''}{efficiency.toFixed(1)}%
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    {!route.mirea && (
                                        <div className="solution-section classical">
                                            <div className="solution-header">
                                                <Route className="solution-icon classical" />
                                                <h4>Classical Solution</h4>
                                                <div className="solution-badge neutral">Not Available</div>
                                            </div>
                                            <div className="no-solution-message">
                                                <p>Classical algorithm solution is not available for this route.</p>
